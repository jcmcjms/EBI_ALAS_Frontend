...
import type { LoanParameters } from "@/src/pages/loans/create/schema";

/**
 * Reactive bridge between RHF's form state and the pure computation
 * engine in `@/src/lib/loan-computations`.
 *
 * Accepts an explicit `LoanParameters` argument (the per-loan parameters
 * object) rather than reading from a singular `loan` form path. This
 * enables one computation engine instance per loan when rendering
 * multi-loan approval forms, where each form needs its own numbers.
 *
 * Still subscribes (via `useWatch`) to the slices the math actually
 * reads — `client`, and the three obligation arrays — so an AO typing
 * into `verification.findings` or `deviations.remarks` does **not**
 * retrigger the recalculation. This keeps the wizard at 60fps even
 * when the printed preview is rendered below the fold.
 *
...
export function useLoanComputations(
    ...
    // path strings which don't require the schema type to be in scope.
