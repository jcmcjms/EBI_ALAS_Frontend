...
export function OverviewTab({ onEditProfile, onOpenTab }: OverviewTabProps) {
    ...
    const contactRows = useMemo(() => {
        const rows: { icon: typeof Envelope; text: string; missing: boolean; key: string }[] = [
            { key: "email", icon: Envelope, text: profile?.email ?? "No email on file", missing: !profile?.email },
            { key: "phone", icon: Phone, text: profile?.phone ?? "No phone on file", missing: !profile?.phone },
            { key: "branch", icon: MapPin, text: branchLabel ?? "No branch assigned", missing: !branchLabel },
            {
                key: "employee",
            ...
            <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
                ...
                <Card>
                    <CardHeader className="flex-row items-center justify-between border-b bg-muted/30 py-3">
                        ...
                        <Button variant="ghost" size="sm" className="gap-1.5" onClick={onEditProfile}>
                ...
                <div className="space-y-6">
                    <Card>
                        <CardHeader className="flex-row items-center justify-between border-b bg-muted/30 py-3">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => onOpenTab("activity")}>
                                View all
                            </Button>
                        </CardHeader>
                        <CardContent className="pt-6">
                            {activityQuery.isLoading ? (
                                ...
                                <ActivityTimeline items={toTimelineItems(activityQuery.data ?? [])} />
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <div className="grid gap-6 xl:grid-cols-2">
                        <Card>
                            <CardHeader className="flex-row items-center justify-between border-b bg-muted/30 py-3">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => onOpenTab("my-applications")}>
                                    View all
                                </Button>
                            </CardHeader>
                                        ...
                                        return (
                                            <div key={loan.id} className="flex items-center gap-3 px-4 py-3">
                                                <div className="min-w-0 flex-1">
                                                    <p className="truncate text-sm font-medium">{loan.lamId}</p>
