...
export function EmptyState({ title, hint, action, icon: Icon = CircleDashed }: EmptyStateProps) {
    return (
        <div role="status" className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <div className="space-y-1">
                ...
                {hint && <p className="mx-auto max-w-[320px] text-xs text-muted-foreground">{hint}</p>}
            </div>
            {action}
        </div>
    );
