import { CheckCircle, ClipboardText, Clock, FloppyDisk, LockSimple, SignIn } from "@phosphor-icons/react";
import type { Activity } from "@/src/lib/api/account";
...
export function toTimelineItems(records: Activity[]): TimelineItem[] {
    return records.map((a) => {
        ...
        if (action.includes("password") || action.includes("login") || action.includes("sign")) {
            ...
            kind = "draft";
        } else if (action.includes("recommend") || action.includes("approv") || action.includes("release")) {
            kind = "approval";
        }
        ...
        return { id: String(a.id), kind, title: `${a.lamId} — ${a.action}`, description, createdAt: a.actionDate };
    });
}
...
export function ActivityTimeline({ items }: { items: TimelineItem[] }) {
    return (
        <ol className="relative space-y-6 before:absolute before:bottom-2 before:left-4 before:top-2 before:w-px before:bg-border">
            {items.map((item) => {
                const KindIcon = ACTIVITY_META[item.kind].icon;
                return (
